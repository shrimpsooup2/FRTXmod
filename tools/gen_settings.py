...
SPEC = [
    ...
    (FLOAT, "bloom-emissive-bias", "emissiveBias", "Emissive Bias",
     ...
     0.0, 1.0, 0.55, 0.01),
    (FLOAT, "bloom-bg-suppress", "bgSuppress", "Background Suppression",
     "Reduces glow in areas whose surroundings are already bright, so a lone bright object keeps its halo while a big washed-out background loses most of it. Raise this when a bright sky is blooming as hard as the objects in front of it.",
     0.0, 1.0, 0.35, 0.01),
    (FLOAT, "bloom-isolation", "isolation", "Isolation Boost",
     "Makes a bright thing surrounded by darkness glow harder than the same brightness in a bright area. Uses the same neighbourhood measurement as Background Suppression, in the opposite direction, and because light rays are built from the glow it gives isolated objects more rays too.",
     0.0, 4.0, 0.8, 0.05),
    (FLOAT, "bloom-bg-radius", "bgRadius", "Background Scale",
     "How wide a neighbourhood counts as 'the background', as a fraction of screen height. Large values only suppress screen-scale washes; small values also dim the middle of big glowing objects.",
     0.02, 0.4, 0.14, 0.01),
    ...
    (FLOAT, "rays-jitter", "raysJitter", "Jitter",
     ...
     0.0, 1.0, 0.9, 0.01),
    (FLOAT, "rays-occlusion", "raysFadeOverDark", "Fade Over Dark",
     "Dims rays over dark parts of the screen. This is a stylistic choice, not a physical one: with no depth buffer the mod cannot tell a solid object from empty background, and in this game the dark areas are usually empty, which is exactly where shafts should show. Leave at 0 unless you want shafts confined to lit areas.",
     0.0, 1.0, 0.0, 0.01),
...
# (enum name, button id, label, which button group)
PRESETS = [
    ("Subtle",      "subtle",      "Subtle",      "core"),
    ("Showcase",    "showcase",    "Showcase",    "core"),
    ("Overkill",    "overkill",    "Overkill",    "core"),
    ("Performance", "performance", "Performance", "core"),
    ("Neon",        "neon",        "Neon",        "style"),
    ("Cinematic",   "cinematic",   "Cinematic",   "style"),
    ("Dreamy",      "dreamy",      "Dreamy",      "style"),
...
def gen_presets_inc():
    out = [
        ...
        "//",
        "// The preset roster. mod.json's buttons, the FRTXPreset enum and the",
        "// tuner's number keys are all built from this, so they cannot disagree.",
        "//",
        "//   FRTX_PRESET(EnumName, \"button-id\", \"Label\")",
        "",
        "#ifndef FRTX_PRESET",
        "#define FRTX_PRESET(name, id, label)",
        "#endif",
        "",
    ]
    for name, ident, label, _group in PRESETS:
        out.append(f'FRTX_PRESET({name}, "{ident}", "{label}")')
    out += ["", "#undef FRTX_PRESET", ""]
    pathlib.Path('src/FRTXPresets.inc').write_text("\n".join(out))
    return len(PRESETS)
...
def gen_mod_json():
    ...
    settings["style-presets"] = collections.OrderedDict([
        ("name", "Styles"),
        ("description", "Different looks rather than different intensities. Neon suits glow-heavy "
                        "levels, Cinematic goes for filmic warmth, Dreamy for a soft wide haze, "
                        "Noir for monochrome contrast, Vivid for punch with little glow, and "
                        "Sunbeam turns on light rays."),
        ("type", "button"),
        ("buttons", collections.OrderedDict(
            [(i, l) for _n, i, l, g in PRESETS if g == "style"])),
    ])
...
if __name__ == '__main__':
    ...
    params = gen_inc()
    presets = gen_presets_inc()
    print(f"src/FRTXPresets.inc: {presets} presets")
